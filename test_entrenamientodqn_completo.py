...
def test_configuracion_real():
    ...
    """
    logger.info("=" * 60)
    logger.info("TEST: CONFIGURACIÓN REAL DE ENTRENAMIENTODQN")
    logger.info("=" * 60)
    ...
    try:
        ...
        config_loader.load_app_settings = lambda: mock_app_settings
        ...
        try:
            ...
            logger.info(f"✅ Instancia creada exitosamente")
            ...
            logger.info(f"📊 Arquitectura: {[layer.units for layer in model.layers if hasattr(layer, 'units')]}")
...
def test_sin_gpu():
    ...
    try:
        ...
        if len(gpus) == 0:
            ...
            logger.warning("⚠️ Aún se ven GPUs (configuración no aplicada completamente)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if test1_ok and test2_ok:
            ...
            logger.info("🚀 Configuración automática de dispositivo funcionando correctamente")
            sys.exit(0)
        else:
