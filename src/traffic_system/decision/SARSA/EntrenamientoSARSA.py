...
class EntrenamientoSARSA:
    ...
    def __setPath(self) -> None:
        ...
        """
        path = (
            f'Decision/Resultados_entrenamiento/SARSA_{time.strftime("%Y-%m-%d_%H-%M")}'
        )
    ...
    def entrenar(
        self, num_epocas: int, alpha: float, gamma: float, epsilon: float
    ) -> None:
        """
        ...
        for epoca in range(num_epocas):
            ...
            if epoca > 0:
                metricas_convergencia = self.__calcular_metrica_convergencia(
                    Q, Q_anterior
                )
            else:
    ...
    def __guardar_metricas(
        self,
        epoca: int,
        duracion: float,
        recompensas_acumuladas: float,
        tasas_exploracion_explotacion: float,
        metricas_convergencia: float,
    ) -> None:
        """
    ...
    def __calcular_metrica_convergencia(self, Q, Q_anterior) -> float:
        ...
        """
        diferencias = [
            (abs(Q[key] - Q_anterior[key])) if key in Q_anterior and key in Q else 0
            for key in Q
        ]
    ...
    def __estado(self) -> tuple:
        ...
        estado = tuple(
            [round(tiempo_espera / tiempo_maximo_espera, 2) for tiempo_espera in estado]
        )
        return estado
    ...
    def __avanzar(self, action: str) -> tuple[tuple, float, bool]:
        ...
        )  #! Estado, recompensa, si terminó la simulacion o no
            ...
            with open(
                ...
                writer.writerow(
                    [
                        ...
                        "Tasa de Exploración vs Explotación (Epsilon)",
